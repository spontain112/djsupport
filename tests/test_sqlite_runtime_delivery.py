...
class TestRuntimeDeliveryContract:
    def test_package_pins_the_only_production_binding_and_python_surface(self):
        ...
        expected_requirement = (
            f"{catalog['selected_binding']['distribution']}=="
            f"{catalog['selected_binding']['version']}"
        )
        assert apsw_dependencies == [expected_requirement]
        assert expected_requirement == "apsw==3.53.4.0"

    def test_versioned_artifact_catalog_is_strict_and_claims_only_proved_cells(self):
        catalog = _load_json(CATALOG_PATH)
    ...
    def test_every_claimed_artifact_has_one_active_runtime_admission(self):
        catalog = _load_json(CATALOG_PATH)
    ...
    def test_native_matrix_uses_clean_binary_installs_and_no_publish_authority(self):
    ...
    def test_runtime_delivery_is_credited_and_monitoring_has_an_owner(self):
        ...
        assert "[certifi](https://github.com/certifi/python-certifi)" in credits
    ...
    def test_repository_owned_install_routes_forbid_apsw_source_builds(self):
        single_line_routes = (
        ...
        for path in single_line_routes:
            ...
            assert commands
            assert all("--only-binary=apsw" in command for command in commands)
        ...
        assert release_guide.count("pip install") == 2
        assert release_guide.count("pip install --only-binary=apsw") == 2


class TestArtifactSelection:
    def test_an_unclaimed_native_cell_has_no_artifact_fallback(self):
    ...
    def test_downloaded_wheel_digest_near_miss_fails_closed(self, tmp_path):
...
class TestPackageArchiveInspection:
