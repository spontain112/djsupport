...
- **Blocking**: use GitHub native issue dependencies. Where dependencies are unavailable, use a `Blocked by: #<n>, #<n>` line at the top of the child body.
- **Frontier query**: list the map's open children, then drop issues with an open blocker or assignee; first in map order wins.
